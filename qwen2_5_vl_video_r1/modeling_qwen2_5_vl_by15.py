...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            ...
            # attn_weights = attn_weights + causal_mask.to(attn_weights.dtype) # to avoid nan in importance_score
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2_5_VLConfig, layer_idx: int, do_pruning=False):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        if not do_pruning:
            ...
            self.self_attn_type = config._attn_implementation
        else:
            self.self_attn = QWEN2_5_VL_ATTENTION_CLASSES["eager"](config, layer_idx)
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
        ...
        for layer_idx in range(config.num_hidden_layers):
            if layer_idx == 0: # the first layer
                if self.retain_rate[0] == full_ratio:
                    self.layers.append(Qwen2_5_VLDecoderLayer(config, layer_idx))
                else:
                    self.layers.append(Qwen2_5_VLDecoderLayer(config, layer_idx, do_pruning=True))
            else: # other layers
                if (self.retain_rate[layer_idx] - self.retain_rate[layer_idx - 1]) == 0:
                    self.layers.append(Qwen2_5_VLDecoderLayer(config, layer_idx))
                else:
                    self.layers.append(Qwen2_5_VLDecoderLayer(config, layer_idx, do_pruning=True))
        # [(l_i, layer.self_attn_type) for l_i, layer in enumerate(self.layers)]
        ############# token pruning #############
    ...
    def forward(
        ...
        ############# Token Merging with Pre-computed Input IDs #############
        ############# token pruning #############
        run_our_forward = boundaries is not None
        only_edit_mask = False 
        ...
        if is_prefill: # prefill phrase
            if run_our_forward:
                ...
                # recompute causal mask if the first layer is eager mode
                if self.layers[0].self_attn_type == 'eager':
            ...
            if importance_score is None: # no importance_score (flash attn layer)
                ...
                if importance_score == 'decoding': # decoding phrase
                    ...
                    if only_edit_mask:
                        ...
                        keep_num = (inds.shape[1] - protected_num) * retain_rate[layer_idx] + protected_num

                        # assumption: batch = 1
                        keep_inds = inds[:, :int(keep_num.max())]
                        # update the atention mask so that next layer will ignore the accumulated, masked tokens
                        token_mask = torch.zeros((hidden_states.shape[0], hidden_states.shape[1])).to(attention_mask.device)
                        ...
                        self.attention_mask_cache[layer_idx + 1] = attention_mask
                        # save boundries for next layer
                        self.boundaries_cache[layer_idx + 1] = boundaries
                    else:
                        # set scores to 1.0 for text tokens that should be protected & set random scores if nan appears so that no protected tokens are pruned
                        ...
                        for b_i, item in enumerate(chosen_hidden_states): # (1, N, C)
                            ...
                            reduct_cnt = inds.shape[1] - int(keep_num[b_i].item())
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        second_per_grid_ts: Optional[torch.Tensor] = None,
        preprocess_boundaries: Optional[torch.LongTensor] = None,
            ...
            if pixel_values is not None: # prefilling stage or RL stage get_per_token_logps()
                if not run_our_forward:
                    ...
                    for b in range(input_ids.size(0)):
                        # assumption: single sample
                        vis_token_id = self.config.image_token_id
                        vis_grid_thw_b = image_grid_thw[b] 
                        this_v_cnt = vis_grid_thw_b.prod().item() // self.config.vision_config.spatial_merge_size // self.config.vision_config.spatial_merge_size
                        encoder_vis_embeds_b = image_embeds[v_encoder_cnt:(v_encoder_cnt+this_v_cnt)].unsqueeze(0) # (1, N, C)
                        v_encoder_cnt += this_v_cnt
                        if b == input_ids.size(0) - 1: # last sample
                            assert v_encoder_cnt == image_embeds.size(0)
                        ...
                        # difference = vis_feats.size(1) - int((H // 2) * (W // 2)) # re-assign position ids
                        ...
                        v_pos_mask = input_ids[b:b+1] == vis_token_id
                        first_nonzero_idx = v_pos_mask.float().argmax(dim=1)
                        # reversed_mask = torch.flip(v_pos_mask, dims=[1])
                        ...
                        all_boundaries.append(boundary_b)
                        
                        # divide chunks of embeddings
                        vis_tok_start_idx = boundary_b[0, 0].item() # assumption
                        q_tok_start_idx = boundary_b[0, 1].item() # assumption
                        sys_embeds = inputs_embeds[b:b+1, :vis_tok_start_idx]
                        # vis_embeds = inputs_embeds[b:b+1, vis_tok_start_idx:q_tok_start_idx]
                        q_embeds   = inputs_embeds[b:b+1, q_tok_start_idx:]

                        # concat input embeddings
                        inputs_embeds_b = torch.cat((sys_embeds, vis_embeds, q_embeds), dim=1)
                        if inputs_embeds_b.size(1) != input_ids[b:b+1].size(1):
                            import pdb; pdb.set_trace()
                        all_inputs_embeds.append(inputs_embeds_b)
                        all_vis_grid_thw.append(vis_grid_thw_b.unsqueeze(0))
                        all_token_idxs.append(token_idx)

                        # convert input_ids to the length before any token reduction
                        sys_ids = input_ids[b:b+1, :vis_tok_start_idx]
                        vis_ids = input_ids[b:b+1, vis_tok_start_idx:q_tok_start_idx]
                        q_ids   = input_ids[b:b+1, q_tok_start_idx:]
                        raw_vis_ids = torch.full((1, T * H * W), vis_token_id, dtype=input_ids.dtype, device=input_ids.device)
                        raw_input_ids_b = torch.cat((sys_ids, raw_vis_ids, q_ids), dim=1)
                        all_raw_input_ids.append(raw_input_ids_b)

                        # convert attention_mask to the length before any token reduction
                        if attention_mask is not None: # prefill stage
                            sys_attnmask = attention_mask[b:b+1, :vis_tok_start_idx]
                            vis_attnmask = attention_mask[b:b+1, vis_tok_start_idx:q_tok_start_idx]
                    ...
                    inputs_embeds = torch.cat(all_inputs_embeds, dim=0) # Qwen2_5_VLProcessor has already align their length     
                    boundaries = torch.cat(all_boundaries, dim=0) 
                    vis_grid_thw = torch.cat(all_vis_grid_thw, dim=0)
                    ############# Token Merging with Pre-computed Input IDs #############

            if pixel_values_videos is not None: # prefilling stage or RL stage get_per_token_logps()
                if not run_our_forward:
                    ...
                    for b in range(input_ids.size(0)):
                        ...
                        vis_grid_thw_b = video_grid_thw[b] 
                        this_v_cnt = vis_grid_thw_b.prod().item() // self.config.vision_config.spatial_merge_size // self.config.vision_config.spatial_merge_size
                        encoder_vis_embeds_b = video_embeds[v_encoder_cnt:(v_encoder_cnt+this_v_cnt)].unsqueeze(0) # (1, N, C)
                        v_encoder_cnt += this_v_cnt
                        if b == input_ids.size(0) - 1: # last sample
                            assert v_encoder_cnt == video_embeds.size(0)
        ...
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            ...
            if (
                ...
                else: ############# Token Merging with Pre-computed Input IDs ############# 
                    position_ids = []
                    rope_deltas = []
                    for b, (token_idx, raw_input_ids, raw_attention_mask) in enumerate(zip(all_token_idxs, all_raw_input_ids, all_raw_attention_masks)):
                        # visual tokens
                        vis_grid_thw_b = vis_grid_thw[b]
                        T = vis_grid_thw_b[0].item()
                        H = vis_grid_thw_b[1].item() // self.config.vision_config.spatial_merge_size # see Qwen2_5_VLPatchMerger
                        W = vis_grid_thw_b[2].item() // self.config.vision_config.spatial_merge_size # see Qwen2_5_VLPatchMerger
                        token_cnt = H * W
                        token_cnt = token_cnt - token_cnt // 2
                        # token_cnt = token_cnt - token_cnt // 2
                        # token_cnt = min(token_cnt, (H // 2) * (W // 2)) # re-assign position ids
                        reduct_cnt = int(H * W - token_cnt) * T # int(H * W - token_cnt) * T # 0 # int(H * W * 0.01) * T # 
                        # text tokens
                        boundary_b = boundaries[b:b+1]
                        vis_tok_start_idx = boundary_b[0, 0].item() # assumption
                        q_tok_start_idx = boundary_b[0, 1].item() # assumption
                        ...
                        position_ids_b, rope_deltas_b = self.get_rope_index(
                            ...
                            second_per_grid_ts=second_per_grid_ts if second_per_grid_ts is None else [second_per_grid_ts[b]],
                        ...
                        vis_position_ids = position_ids_b[:, :, vis_tok_start_idx:q_tok_start_idx][:, :, token_idx]
                        # vis_position_ids = position_ids_b[:, :, vis_tok_start_idx:q_tok_start_idx][:, :, :int(token_cnt * T)] # re-assign position ids
                        q_position_ids = position_ids_b[:, :, q_tok_start_idx:]
                        position_ids_b = torch.cat((sys_position_ids, vis_position_ids, q_position_ids), dim=-1) # (3, B, L)
                        position_ids.append(position_ids_b)
                    position_ids = torch.cat(position_ids, dim=1) # (3, B, L)
                    self.rope_deltas = torch.cat(rope_deltas, dim=0) # (B, 1)
                    ############# Token Merging with Pre-computed Input IDs #############
        ...
        outputs = self.model(
            ...
            vis_grid_thw=vis_grid_thw,
            rope_deltas=self.rope_deltas.clone(),
        )
        ...
        if run_our_forward:
            ...
            updated_logits = None
            if rl_forward: # RL stage get_per_token_logps()
                # remove visual tokens
                ids_wo_vis = []
                for b_i in range(input_ids.shape[0]):
                    boundary = boundaries[b_i]
                    ids_wo_vis_b = torch.cat((input_ids[b_i:b_i+1, :boundary[0]], input_ids[b_i:b_i+1, boundary[1]:]), dim=-1)
                    ids_wo_vis.append(ids_wo_vis_b)
                updated_input_ids = torch.cat(ids_wo_vis, dim=0)
                # inject pseudo visual logits
                # logits_w_vis = []
                # for b_i in range(logits.shape[0]):
                #     boundary = boundaries[b_i]
                #     sys_logits = logits[b_i:b_i+1, :boundary[0]]
                #     pseudo_logits = logits.new_full((1, boundary[1]-boundary[0], logits.shape[2]), fill_value=1)
                #     q_completion_logits = logits[b_i:b_i+1, boundary[0]:]
                #     logits_w_vis_b = torch.cat((sys_logits, pseudo_logits, q_completion_logits), dim=1)
                #     logits_w_vis.append(logits_w_vis_b)
                # updated_logits = torch.cat(logits_w_vis, dim=0)
        ...
        model_inputs.update(
            {
                ...
                "preprocess_boundaries": preprocess_boundaries,
            }
        )
        return model_inputs
...
def gumbel_softmax(
    ...
    """
    def gumbel_noise(shape, eps=1e-10):
        U = torch.rand(shape)
        return -torch.log(-torch.log(U + eps) + eps)
    
    gumbels = gumbel_noise(logits.size())
    ...
    if hard: # Straight-through: hard in forward, soft in backward
        ...
        # Get top-k indices
        topk_vals, topk_idx = torch.topk(gumbels, k, dim=-1)
        # Construct mask
        y_hard = torch.zeros_like(logits).scatter_(1, topk_idx, 1.0) # multi-hot target
        ret = y_hard - y_soft.detach() + y_soft
    else: # Soft relaxation with softmax (continuous weights instead of hard 0/1)
        ret = y_soft
    return ret
