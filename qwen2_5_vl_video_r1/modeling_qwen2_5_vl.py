    ...
    def forward(
        ...
        if is_prefill: # prefill phrase
            if run_our_forward:
                ...
                if not rl_forward: # for leanable token sampling, keep record of chosen indices so that it can be recovered next time
                    self.gumbel_noise_cache = [None for _ in range(len(self.layers) + 1)]
                ...
                if importance_score is None: # no importance_score (flash attn layer) # 只有部分layer做pruning，初始化会定义好需要pruning的layer # 这个用flash attn
                    ...
                    if importance_score == 'decoding': # decoding phrase
                        ...
                        if only_edit_mask: # false，旧实现
                            ...
                            keep_num = (inds.shape[1] - protected_num) * self.retain_rate[layer_idx] + protected_num
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        run_our_forward = self.model.run_our_forward
        all_inputs_embeds = []
        all_boundaries = []
        all_vis_grid_thw = []
        all_token_idxs = []
