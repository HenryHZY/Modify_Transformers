    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs = self.self_attn(
            ...
            attention_mask=attention_mask, # None for flash attn # TODO: use eager mode for debug?
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if position_ids.ndim == 3 and position_ids.shape[0] == 4:
            ...
            text_position_ids = position_ids[0] # 进入这个，temporal pos

        # TODO: 这里会覆盖之前的 attention_mask
        attention_mask = create_causal_mask(
            ...
            past_key_values=past_key_values, 
            position_ids=text_position_ids, # torch.Size([1, 330]), text's rope 
        )
        import pdb; pdb.set_trace()
        
        # # TODO: 直接复制的之前的函数，是否可用？最开始得到 causal_mask = None正常，后续呢？
        # output_attentions=False
        # causal_mask = self._update_causal_mask(
        #     attention_mask=attention_mask, # torch.Size([1, 330]) 
        #     input_tensor=inputs_embeds, # torch.Size([1, 330, 4096])
        #     cache_position=cache_position, # torch.Size([330]), tensor([  0,   1,   2, ..., 329]
        #     past_key_values=past_key_values, # DynamicCache(layers=[DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer, DynamicLayer])
        ...
        # decoder layers
        for layer_idx, decoder_layer in enumerate(self.layers): # LLM开始, each layer
            ...
            # import pdb; pdb.set_trace() 
            # check hidden_states shape, 
            hidden_states = layer_outputs[0] # torch.Size([1, 330, 4096])

            # check deepstack_visual_embeds
    ...
    # TODO: 'Qwen3VLTextConfig' object has no attribute 'sliding_window'
    def _update_causal_mask(
        ...
        # TODO: fix AttributeError: 'Qwen3VLTextConfig' object has no attribute 'sliding_window'
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        run_our_forward = self.language_model.run_our_forward
        ...
        if pixel_values is not None:   # TODO: 图片代码后续参考video
        ...
        if position_ids is None:
            ...
            if (prefill_compiled_stage or prefill_noncompiled_stage) or self.rope_deltas is None:                
                ...
                if not run_our_forward: # original network forward
                    ...
                    q_start_idx_merged = boundaries[1] # e.g., 321 # TODO: 修改q_start_idx
                    ...
                    temp_input_ids = raw_input_ids # TODO: 展示先注释掉这些，后续RL再看看
                    ...
                    # TODO: 其他代码地方也要记得兼容image和video的处理
                    # import pdb; pdb.set_trace()
                    # 非视觉token
        ...
        outputs = self.language_model( # TODO: 对应video-r1 qwen2_5_vl_video_r1/modeling_qwen2_5_vl.py的self.model()
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # TODO: qwen2.5vl video-r1用的 self.model(input_ids=None)
        ...
        if labels is not None:
            if outputs.run_our_forward: # 零散的辅助操作，比如modify data type
                ...
                first_layer_input_b = self.model.boundaries_cache[0][0] # TODO: 此处也应该使用修改后的boundary定义
        ...
        if outputs.run_our_forward:
            ...
            if outputs.rl_forward: # RL stage get_per_token_logps()
                ...
                for b_i in range(input_ids.shape[0]):
                    ...
                    ids_wo_vis_b = torch.cat((input_ids[b_i:b_i+1, :boundary[0]], input_ids[b_i:b_i+1, boundary[1]:]), dim=-1) # TODO: 此处也应该使用修改后的boundary定义
...
def get_protected_info_old_backup(boundaries, hidden_states): # TODO: bs=1时，这个函数的作用？
