...
class Qwen3VLTextDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        #   attention_mask = None for flash attn + token pruning【默认是这个，debug也是为了这个】
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #     attention_mask=attention_mask, # torch.Size([1, 330])，全为1
        #     past_key_values=past_key_values,
        ...
        #     deepstack_visual_embeds=deepstack_visual_embeds, # len=3, deepstack_visual_embeds[0].shape = torch.Size([240, 4096]) # NOTE-ZY：已经进行了token selection
        ...
        # 正常情况下，若使用attn_implementation="flash_attention_2"：
            # 在prefill stage时，past_key_values为空 (past_key_values.get_seq_length() = 0), causal_mask = None
            # 在decoding stage时，past_key_values非空 (past_key_values.get_seq_length() != 0), causal_mask = None
        # 正常情况下，若使用attn_implementation="eager"：
            # 在prefill stage时，past_key_values为空 (past_key_values.get_seq_length() = 0), causal_mask != None -> torch.Size([1, 1, 330, 330])
            # 在decoding stage时，past_key_values非空 (past_key_values.get_seq_length() != 0), causal_mask != None -> TODO: ？
        ...
        if is_prefill: # prefill stage
            ...
            # import pdb; pdb.set_trace()
            # first pdb at decoding stage: # 与video-r1相符
            # attention_mask.shape = torch.Size([1, 331]) # 全为1的mask
            # inputs_embeds.shape = torch.Size([1, 1, 4096]) # 相对于prefillshape改变 TODO: 只能看见当前token？
            # cache_position.shape = torch.Size([1]); tensor([  330]) # 数值递增，相对于prefillshape改变
            # past_key_values.get_seq_length() = 330
            # causal_mask = None

            # seconds pdb at decoding stage:
            # attention_mask.shape = torch.Size([1, 332]) -> # TODO: shape递增? 好像有点问题？
            # inputs_embeds.shape = torch.Size([1, 1, 4096])
            # cache_position.shape = torch.Size([1]); tensor([  331]) # 数值递增
            # past_key_values.get_seq_length() = 331 # 数值递增
            # causal_mask = None            

        hidden_states = inputs_embeds # torch.Size([1, 330, 4096])

        # create position embeddings to be shared across the decoder layers
        position_embeddings = self.rotary_emb(hidden_states, position_ids) # NOTE-ZY: 此处rope pos是merging后的，然后得到相应的pos emb
            # len(position_embeddings) = 2,
            # position_embeddings[0].shape = position_embeddings[1].shape = torch.Size([1, 330, 128]) 对应 cos, sin
        ...
        if is_prefill: # prefill phrase
            if run_our_forward:
                ...
                protected_inds, protected_num = get_protected_info(boundaries, hidden_states) # NOTE-ZY: 作用是标记非视觉token位置。函数实现需要boundary定义调整。used in prefill stage, not used in decoing stage
        ...
        for layer_idx, decoder_layer in enumerate(self.layers): # NOTE-ZY: LLM内的每一层，需要考虑prefill / decoding stage，是否切换eager / flash attn
            # if layer_idx == 17 or layer_idx == 18 or layer_idx == 19 or layer_idx == 22 or layer_idx == 23: # flash attn -> eager attn -> eager attn -> flash attn
                ...
                # text_position_ids.shape = torch.Size([1, 282]) # NOTE-ZY: fix, 这个update
                # past_key_values.get_seq_length() = 330
            ...
            if deepstack_visual_embeds is not None and layer_idx in range(len(deepstack_visual_embeds)): 
                ...
                # 相当于前三层hidden_states的视觉特征（通过visual_pos_masks找到视觉特征）的残差连接 -> hidden_states[visual_pos_masks, :] += deepstack_visual_embeds[layer_idx]
            ...
            ############# token pruning #############
            if run_our_forward: 
                # 开始选token, key tokens, score = attn_w，具体计算有现成func 
                ...
                if importance_score is None: 
                    # importance_score is None -> 当前layer是flash attention的layer -> 但是需要分别在prefill 和 decoding stage时，为下一层是eager attention的layer做衔接，生成要求的causal attention
                    # 【如果是纯eager，会OOM】
                    if is_prefill: # prefill phrase or RL stage get_per_token_logps()
                        ...
                        batch_size, seq_length, _ = hidden_states.shape # torch.Size([1, 1, 4096]) TODO: decoding的hidden_states如何得到？
                        attention_mask = torch.cat((attention_mask, attention_mask.new_full((attention_mask.shape[0], seq_length), fill_value=1)), dim=-1) 
                            ...
                            # input attention_mask.shape = torch.Size([1, 330]), 全为1
                            # output attention_mask.shape = torch.Size([1, 331]), 全为1
                    ...
                    if importance_score == 'decoding': # decoding phrase # TODO: 先看prefill stage
                        ...
                        keep_num = int(((v_cnt_before_llm * self.retain_rate[layer_idx]).long() + protected_num).item()) # (importance_score.shape[1] - protected_num) * self.retain_rate[layer_idx] + protected_num
                            # v_cnt_before_llm = 240, self.retain_rate[layer_idx] = 0.8 -> 240*0.8 = 192 # NOTE-ZY: 这里 v_cnt_before_llm 定义是输入的visual token总数
                            # protected_num = 90 # 非视觉token NOTE-ZY: 这里面包括了 <vstart> <vend>, 因为240+90=330
                            # keep_num = 192+90 = 282
                        assert hidden_states.shape[0] == 1, f"Batch size > 1 (bs={hidden_states.shape[0]}) 暂不支持"
                        ...
                        importance_score[0, protected_token_indices] = 1.0 
                            ...
                            # set scores to 1.0 for text tokens that should be protected -> 因为降序，textual token必定能被保留
                        _, inds = torch.sort(importance_score, dim=-1, descending=True) # 降序，选择要被pruning token
                        ...
                        for v_start, v_end in adjusted_boundaries[0]:
                            # v_start 的新位置，取决于 *在它之前* 删除了多少 token。
                            # 此时的 cumulative_reduct_cnt 正是这个值 (即上一轮循环结束时的总数)。
                            new_v_start = v_start - cumulative_reduct_cnt
                            # 计算 *当前* 帧内部 (v_start + 1, v_end) 有多少 token 被删除了
                            ...
                            # v_end 的新位置，取决于 *到目前为止* (包括当前帧) 总共删除了多少 token
                        ...
                        reduct_cnt = importance_score.shape[1] - hidden_states.shape[1] 
                            # 330 - 282 = 48, 都是从visual token中pruning的
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # import pdb; pdb.set_trace()
        # 核心的token merging # LLM
        if inputs_embeds is None:
            inputs_embeds = self.get_input_embeddings()(input_ids) # 已经进行过token compression的input_ids -> 已经进行过token compression的inputs_embeds -> torch.Size([1, 330, 4096])

        image_mask = None
        video_mask = None
        
        # prefilling stage or RL stage get_per_token_logps() # 图片和video操作基本一致，似乎直接复制就行
        ...
        if pixel_values_videos is not None: # 视频
            if not run_our_forward: # original code
                ...
                video_embeds = torch.cat(video_embeds, dim=0).to(inputs_embeds.device, inputs_embeds.dtype)
                    # torch.Size([960, 4096]) 未进行token merging的video_embeds
                ...
                for i in range(T):
                    ...
                    # inputs_embeds_b
                    updated_embeds_chunks.append(inputs_embeds[0, last_idx:start_idx])      # 添加此 video chunk 之前的 文本部分 (e.g., <|im_start|>user\n<19.6 seconds>)
                    updated_embeds_chunks.append(inputs_embeds[0, start_idx:start_idx+1])   # <|vision_start|>
                    updated_embeds_chunks.append(merged_embeds_per_frame[i])                # token compression后的30个<|video_pad|>
                    updated_embeds_chunks.append(inputs_embeds[0, end_idx:end_idx+1])       # <|vision_end|>
                    
                    # raw_input_ids_b
                    updated_raw_ids_chunks.append(input_ids_1d[last_idx:start_idx])
                    updated_raw_ids_chunks.append(input_ids_1d[start_idx:start_idx+1])
                    updated_raw_ids_chunks.append(raw_vis_ids_per_frame)                    # 未进行token compression的120个<|video_pad|>的input_ids
                    updated_raw_ids_chunks.append(input_ids_1d[end_idx:end_idx+1])
                    ...
                    if attention_mask_1d is not None:
                        updated_raw_mask_chunks.append(attention_mask_1d[last_idx:start_idx])
                        updated_raw_mask_chunks.append(attention_mask_1d[start_idx:start_idx+1])
                        updated_raw_mask_chunks.append(raw_vis_mask_per_frame)              # 未进行token compression的120个<|video_pad|>的masks
                        updated_raw_mask_chunks.append(attention_mask_1d[end_idx:end_idx+1])
        ...
        # NOTE-ZY: deepstack会受token compression影响 -> deepstack visual features没有进行merging，长度不匹配
        # NOTE-ZY: 前面token compression影响导致 image_mask = None / video_mask = None
        ...
        if not run_our_forward: # original code
            ...
            if pixel_values_videos is not None:
                # NOTE-ZY: deepstack_visual_embeds筛选，暂时用selection的方式，没用pooling
                ...
                deepstack_visual_embeds = [layer_embeds[token_idx] for layer_embeds in deepstack_video_embeds] 
                    # deepstack_video_embeds[0].shape = torch.Size([960, 4096])，每层deepstack_video_embeds都进行相同的index选择
        ...
        if position_ids is None: # NOTE-ZY: 初始化的position_ids = None -> 后面使用get_rope来获取3D position_ids
            ...
            if (prefill_compiled_stage or prefill_noncompiled_stage) or self.rope_deltas is None:                
                # prefill stage
                # if len(all_raw_input_ids) == 0: # NOTE-ZY: 考虑换一种condition
                if not run_our_forward: # original network forward
                    ...
                    # NOTE-ZY: Simply use bs=1 for each gpu during training & inference
                    assert input_ids.shape[0] == 1, f"Batch size > 1 (bs={input_ids.shape[0]}) 暂不支持"
                    # token_idx.shape = torch.Size([240]) # video token的位置
                    # raw_input_ids.shape = torch.Size([1, 1050]) # 用于提取原始的rope pos
                    ...
                    # q_tok_start_idx_raw 是 *raw* (1050) 序列中 query token 的起始索引
                    ...
                    temp_input_ids = raw_input_ids # TODO: 展示先注释掉这些，后续RL再看看有无问题
                    if raw_attention_mask is None: # RL stage get_per_token_logps()
                        # NOTE-ZY: 需要根据boundary调整，得到正确的completion_ids
                        # sys_vis_ids = raw_input_ids[:, :q_tok_start_idx_raw] # system & visual tokens （没有包括最后一个 <|vision_end|>）
                        q_completion_ids = raw_input_ids[:, q_tok_start_idx_raw:] # question tokens & completion tokens （这里面没有视觉token即可）
                    ...
                    # position_ids_b.shape = torch.Size([3, 1, 1050])，未进行merging的pos ids
                    ...
                    self.rope_deltas = rope_deltas_b 
                        # NOTE-ZY: rope_deltas_b = -840+720 = -120 仍为负数. Pruning的时候会使用上，因为生成新tokens -> 重新算pos 类似于pos offset

                    # NOTE-ZY: 修改正确的pos index
                    # import pdb; pdb.set_trace()
                    # 非视觉token
                    ...
                    # 获取那 960 个 视觉 token 在 [1, 1050] 序列中的 *绝对索引*
                    raw_vis_indices_abs = raw_visual_pos_masks.nonzero(as_tuple=True)[1] # Shape [960], tensor([  11,   12,   13,   14, ..., 1037, 1038, 1039]
                    # token_idx (Shape [240]) 包含我们想*保留*的 token 的 *相对索引* (0-959)，用它来从 960 个绝对索引中选出 240 个要保留的
                    kept_vis_indices_abs = raw_vis_indices_abs[token_idx] # Shape [240], tensor([  14,   18,   22, ..., 1031, 1035, 1039], # 跟token_idx对应
                    # 创建一个 *新* 的 mask，只标记我们*保留*的 240 个 视觉 token
                    kept_vis_mask = torch.zeros_like(raw_visual_pos_masks) # Shape [1, 1050], 全 False
                    kept_vis_mask[0, kept_vis_indices_abs] = True # 将 240 个位置设为 True
                    # 合并 text_mask 和 kept_vis_mask，(所有文本 + 保留的视觉) = 最终的 330 个 token
                    final_selection_mask = raw_non_visual_mask | kept_vis_mask # Shape [1, 1050]
                    # 从 final_selection_mask 中获取最终的*索引*
                    final_selection_indices = final_selection_mask.nonzero(as_tuple=True)[1] # Shape [330], tensor([0, 1, ...,8, 9, 10, 14, 18, 22, ...., 1047, 1048, 1049]
                    # 使用这些索引从 position_ids_b 中选择
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs = self.model( # 把merge后的token放进LLM 1/4 num tokens
            ...
            attention_mask=attention_mask, # e.g., torch.Size([1, 330]), 全为1的matrix
...
def get_protected_info(boundaries, hidden_states):
    ...
    for (start_idx, end_idx) in vision_start_end_idx_pairs:
        ...
        # 3. [NEW] 添加 <|vision_end|> token (它也是非视觉 token)
        #    (跳过 [start_idx + 1, end_idx - 1] 之间的视觉 pad token)
        end_token_chunk = torch.arange(end_idx, end_idx + 1, device=hidden_states.device)
        this_inds_col_chunks.append(end_token_chunk)

        # 4. 更新 current_idx，跳过这个 visual chunk
        current_idx = end_idx + 1
    
    # 5. 添加最后一个 visual chunk 之后的 文本 (query) token
    ...
    # 将所有文本块（syn、vstart、vend、text pad、Query）拼接在一起
    this_inds_col = torch.cat(this_inds_col_chunks).unsqueeze(1) 
        # torch.Size([90, 1]), tensor([[  0], [  1], ... [ 10], [ 41],  [329]], device='cuda:0') 
        # 对应vision_start_end_idx_pairs，不包括visual token
        # 90 = 330-30*8
    
    # 添加 batch index (确保在同一设备上)
